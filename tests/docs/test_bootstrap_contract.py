...
def test_bootstrap_contract_entrypoints_match_runtime_surface():
    ...
    assert {"signal-graph", "signal-graph-mcp", "signal-graph mcp-server"} <= entrypoint_names
...
def test_docs_reference_bootstrap_contract_and_command_surface():
    texts = {
        ...
        "analyst_agent": Path("docs/runbooks/analyst-agent-guide.md").read_text(),
    ...
    assert "next actions" in texts["analyst_agent"].lower()
